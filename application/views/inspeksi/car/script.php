<script type="text/javascript">
    ...
    function getInspeksi() {
        $.ajax({
            type: "POST",
            url: "<?= base_url('Inspeksi/InspeksiCar/getInspeksi') ?>",
            dataType: "JSON",
            success: function(response) {
                if ($.fn.DataTable.isDataTable('#carInspeksiTable')) {
                    $('#carInspeksiTable').DataTable().clear();
                    $('#carInspeksiTable').DataTable().destroy();
                }

                $('#carInspeksiTable > tbody').empty();
    ...
    function hapusInspeksi(idInspeksi) {
        confirmAlert('Apakah anda yakin?', 'Tekan ya untuk menghapus dan tidak untuk kembali!', 'warning', 'Ya, hapus', 'Tidak').then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "<?= base_url('Inspeksi/InspeksiCar/hapusInspeksi') ?>",
                    data: {
                    ...
                    success: function(response) {
                        if (response == 1) {
                            ...
                            setTimeout(() => {
                                window.location.href = "<?= base_url('Inspeksi/InspeksiCar') ?>"
                            }, 2000);
                        } else {
    ...
    $("#btnNextPage").on('click', function() {
        var tglWaktuInspeksi = $('#tglInspeksi').val();
        var shift = $('#shift').val();
    ...
    $('#btnsaveinspeksi').on('click', function() {
        ...
        confirmAlert('Apakah anda yakin?', 'Pastikan data yang anda input benar!', 'warning', 'Ya, simpan', 'Tidak').then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "<?= base_url('Inspeksi/InspeksiCar/saveInspeksi') ?>",
                    data: form_data,
                    contentType: false,
                    processData: false,
                    dataType: "JSON",
    ...
    $('#updatebtnsaveinspeksi').on('click', function() {
        ...
        confirmAlert('Apakah anda yakin?', 'Pastikan data yang anda input benar!', 'warning', 'Ya, simpan', 'Tidak').then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "<?= base_url('Inspeksi/InspeksiCar/updateInspeksi') ?>",
                    data: form_data,
                    contentType: false,
                    processData: false,
                    dataType: "JSON",
